...
static uint8_t debug_rx_fifo_buff[DEBUG_RX_FIFO_SIZE];
...
/**    
  ...
  * @retval 无    
  */
static void bsp_uart_register(bsp_uart_t *uart, fifo_t *fifo, uint8_t *tx_buff, uint8_t *rx_buff)
{
    uart->rx_finish = 0;
    uart->tx_count = 0;
    uart->rx_count = 0;
...
{  
    ...
    /* 注册UART句柄及其发送和接收缓冲区 */  
    bsp_uart_register(&debug, &debug_fifo, debug_tx_buff, debug_rx_buff);  
...
void USER_UART1_IRQHandler(void)
{
    ...
    if(RESET != LL_USART_IsActiveFlag_IDLE(USART1))	/* 检查USART1是否检测到IDLE标志 */ 
    {
		...
		//转到FIFO
		fifo_write(debug.rx_fifo, debug.rx_buff , debug.rx_count);	
