...
//float 单精度浮点数占用32bit空间，不像存储整型或char型数据那么便捷
//我们在存储float数据时就需要先将float数变换一下，变换成数组的格式，这样就方便存储和读取了
//这就是利用了共用体共用一段内存的原理，即 float data 和 uint8_t temp[4]，使用的是一个空间大小的内存。
